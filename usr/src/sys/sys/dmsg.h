...
/*
 * OpenBSD userland port note: this is the DragonFly on-wire dmsg protocol
 * header (from the port kernel sys/hammer2/dmsg2.h).  It still carries the
 * kernel struct globaldata/thread definitions; they are harmless in userland
 * once MAXCOMLEN and the value-type headers below are available.  <uuid.h>
 * supplies the complete struct uuid / uuid_t used by the on-wire structs.
 */
#include <sys/types.h>
#include <sys/time.h>
#include <sys/queue.h>
...
struct dmsg_hdr {
	...
	uint64_t	link_verifier;	/* 18 link-layer verifier (was reserved18) */
...
#define DMSG_PEER_ROUTER	1	/* cluster controller / router (alias) */
#define DMSG_PEER_BLOCK		2	/* block devices */
#define DMSG_PEER_HAMMER2	3	/* hammer2-mounted volumes */
#define DMSG_PEER_CLIENT	4	/* generic client (libdmsg) */
